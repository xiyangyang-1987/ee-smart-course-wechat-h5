(function(){
  ...
  function renderAnalysis(analysis){
    ...
    box.innerHTML='<div class="banner '+(analysis.score>=85?'good':analysis.score>=60?'warn':'bad')+'"><b>结构评分 '+analysis.score+'/100 · '+esc(analysis.level)+'</b><br><small>'+esc(analysis.filename)+' · 组件 '+analysis.componentCount+' · 网络覆盖 '+Math.round((analysis.connectionRate||0)*100)+'%</small><br><small>该分数用于定位课程设计证据缺口，不是课程正式成绩。</small></div>'+
      '<div class="metrics"><div class="metric"><b>'+counts.clocks+'</b><span>时基/时钟</span></div><div class="metric"><b>'+counts.counters+'</b><span>计数器</span></div><div class="metric"><b>'+counts.displays+'</b><span>显示/译码</span></div><div class="metric"><b>'+counts.andGates+'</b><span>与门回零线索</span></div></div>'+
  ...
  function renderBatch(items){
    ...
    box.innerHTML='<div class="banner"><b>批量读取 '+items.length+' 份课程文件 · 平均结构评分 '+average+'/100</b><br><small>评分用于筛查结构与证据缺口，不是课程正式成绩。点击文件名查看系统问题和建议。</small></div><div class="table-wrap"><table><thead><tr><th>文件</th><th>评分</th><th>组件</th><th>计数器</th><th>显示</th><th>网络覆盖</th><th>主要问题</th></tr></thead><tbody>'+items.map((x,i)=>'<tr><td><button class="link" data-analysis-index="'+i+'">'+esc(x.filename)+'</button></td><td><b>'+x.score+'</b></td><td>'+x.componentCount+'</td><td>'+x.counts.counters+'</td><td>'+x.counts.displays+'</td><td>'+Math.round(x.connectionRate*100)+'%</td><td>'+esc(x.questions[0]||'提交边界波形供教师复核')+'</td></tr>').join('')+'</tbody></table></div><div id="batch-detail"></div>';
