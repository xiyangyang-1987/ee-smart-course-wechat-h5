/* electronics-workbench-decoder 0.2.0 adaptation; ISC license */
(() => {
  // ms14_decoder_entry.js
  var BrowserBuffer = class _BrowserBuffer extends Uint8Array {
    static from(input, encoding) {
      if (typeof input === "string") {
        if (encoding === "latin1" || encoding === "ascii") {
          const out = new _BrowserBuffer(input.length);
          for (let i = 0; i < input.length; i += 1) out[i] = input.charCodeAt(i) & 255;
          return out;
        }
        return new _BrowserBuffer(new TextEncoder().encode(input));
      }
      if (input instanceof ArrayBuffer) return new _BrowserBuffer(new Uint8Array(input));
      if (ArrayBuffer.isView(input)) return new _BrowserBuffer(input.buffer, input.byteOffset, input.byteLength);
      return new _BrowserBuffer(input || []);
    }
    static allocUnsafe(length) {
      return new _BrowserBuffer(length);
    }
    static isBuffer(input) {
      return input instanceof _BrowserBuffer;
    }
    static concat(list) {
      const total = list.reduce((sum, x) => sum + x.length, 0);
      const out = new _BrowserBuffer(total);
      let offset = 0;
      for (const x of list) {
        out.set(x, offset);
        offset += x.length;
    ...
    subarray(start, end) {
      ...
      return new _BrowserBuffer(view.buffer, view.byteOffset, view.byteLength);
    }
    equals(other) {
    ...
    readUIntLE(offset, width) {
      let value = 0;
      for (let i = 0; i < width; i += 1) value += this[offset + i] * 2 ** (8 * i);
      return value;
    }
    toString(encoding) {
      if (encoding === "ascii" || encoding === "latin1") return String.fromCharCode(...this);
      return new TextDecoder().decode(this);
    }
  };
  var Buffer = BrowserBuffer;
  ...
  var FORMATS = [
    ...
    },
    {
      ...
      header: "\0\0\0Standard Jet DB",
      extensionPattern: /\.(prj|usr)$/i,
      kind: "mdb"
    }
  ];
  var MAX_HEADER_LENGTH = Math.max(...FORMATS.map((f) => f.header.length));
  function detectFormatByHeader(buffer) {
    return [...FORMATS].sort((a, b) => b.header.length - a.header.length).find((f) => {
  ...
  var UnexpectedValue = class extends Error {
    expected;
    received;
  ...
  function decodeBlock(block, expectedLength) {
  ...
  function decodeBuffer(input) {
    ...
    if (!format) {
      throw new UnexpectedValue(`Input doesn't match any known EW format. Supported: ${knownFormatsList()}`, `one of ${knownFormatsList()}`, header.toString("ascii"));
    }
    if (format.kind !== "compressed-xml") {
      throw new Error(`Input is "${format.label}" (kind=${format.kind}); decodeBuffer only handles compressed-xml. Use decodeMdbBuffer for ${format.kind} containers.`);
    }
  ...
  window.MS14Decoder = { decode(input) {
    const result = decodeBuffer(Buffer.from(input));
    return { format: result.format, xml: new TextDecoder("utf-8").decode(result.xml) };
  } };
})();
